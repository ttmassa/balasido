...
export class Player extends TransformNode {
    ...
    constructor(assets, scene: Scene, shadowGenerator: ShadowGenerator, input?) {
        ...
        //Détection de la plateforme finale qui déclenchera les feux d'artifice
            ...
            new ExecuteCodeAction(
                ...
                () => {
                    if (this.lanternsLit == 22) {
                        this.win = true;
                        //On tourne la caméra vers où les feux d'artifices seront lancés
        ...
        //Détection du sol du monde pour enrengistrer la dernière safe position du joueur et le réanimer à cet endroit quand il tombe
        this.mesh.actionManager.registerAction(
            new ExecuteCodeAction(
                {
                    trigger: ActionManager.OnIntersectionEnterTrigger,
                ...
                () => {
                    this.mesh.position.copyFrom(this._lastGroundPos) //On a besoin d'utiliser une copie où ils vont pointer les 2 au même endroit et se mettront à jour entre eux 
    ...
    private _updateCamera(): void {
        ...
        //Endroits où il faut tourner la caméra
        if (this.mesh.intersectsMesh(this.scene.getMeshByName("cornerTrigger"))) {
            ...
            //On tourne la caméra vers le bas quand le joueur rentre dans lieu, et on la remet normal quand il en sort
            ...
            //Maintenant on fait l'inverse pour quand le joueur quitte le lieu
            if (this.mesh.intersectsMesh(this.scene.getMeshByName("destinationTrigger"))) {
                if (this._input.verticalAxis > 0) {
                    this._yTilt.rotation = Vector3.Lerp(this._yTilt.rotation, Player.ORIGINAL_TILT, 0.4);
                } else if (this._input.verticalAxis < 0) {
